...
```
A regression model that includes three pollutants has additive slope coefficients. The model predicts that for each increase of 10^x in NOX, HC, and SO2 respectively there is a 134x, -132x, and 27.1x change in mortality, respectively. However, the 95% confidence interval for SO2 includes 0, so it is not a reliable predictor. 
...
```{r, echo=FALSE, fig.width=12, fig.height=6}
x_orig <- data.frame(log_nox = pollution$log_nox[1:29], 
                     log_hc = pollution$log_hc[1:29],
                     log_so2 = pollution$log_so2[1:29])

y <- pollution$mort[1:29]

model3_half <- lm(y ~ log_nox + log_hc + log_so2, data=x_orig)

x_new <- data.frame(log_nox = pollution$log_nox[30:60], 
                    log_hc = pollution$log_hc[30:60],
                    log_so2 = pollution$log_so2[30:60])

model_3b_pred <- predict(model3_half, x_new)

ggplot(data = NULL, aes(x = pollution$mort[30:60], y = model_3b_pred)) +
  geom_point() +
...
drinks$drinks <- suppressWarnings(factor(drinks$drinks , levels = drinks$drinks [order(-drinks$prop)]))
...
drugs$drugs <- suppressWarnings(factor(drugs$drugs , 
                                       levels = drugs$drugs [order(-drugs$prop)]))
...
ggplot(data=profiles, aes(x=has_makeup, y=is_female)) + 
  xlab("has makeup") + ylab("is female?") +
  geom_jitter()

#bakes
profiles$has_baking <- profile.has.query(data.frame = essays, query = "baking")
group_by(profiles, has_baking) %>% 
  summarise(prop_female=mean(is_female)) %>% kable(., digits = 3)

ggplot(data=profiles, aes(x=has_baking, y=is_female)) + 
  xlab("has baking") + ylab("is female?") +
  geom_jitter()

#heels
profiles$has_heels <- profile.has.query(data.frame = essays, query = "heels")
group_by(profiles, has_heels) %>% 
  summarise(prop_female=mean(is_female)) %>% kable(., digits = 3)

ggplot(data=profiles, aes(x=has_heels, y=is_female)) + 
  xlab("has heels") + ylab("is female?") +
  geom_jitter()

#brunch
profiles$has_brunch <- profile.has.query(data.frame = essays, query = "brunch")
group_by(profiles, has_brunch) %>% 
  summarise(prop_female=mean(is_female)) %>% kable(., digits = 3)

ggplot(data=profiles, aes(x=has_brunch, y=is_female)) + 
  xlab("has heels") + ylab("is female?") +
  geom_jitter()
